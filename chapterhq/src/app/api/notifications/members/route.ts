...
const querySchema = z.object({ search: z.string().optional(), committeeId: z.string().min(1).optional() });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const committeeId = isOrganizationAdministrator ? query.committeeId : context.activeCommitteeId;
    if (!committeeId || (!isOrganizationAdministrator && query.committeeId && query.committeeId !== committeeId)) {
